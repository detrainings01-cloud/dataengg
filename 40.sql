CREATE DATABASE TEST; 


CREATE TABLE ORDERS (
	ORDER_ID INTEGER  
	, CUSTOMER_ID  INTEGER 
	, PRODUCT_ID INTEGER 
	, PRICE   NUMERIC(10,2)
	, QUANTITY  INTEGER 
);

CREATE TABLE PRODUCTS(
	PRODUCT_ID INTEGER 
	, PRODUCT_NAME VARCHAR(250) 
	, CATEGORY VARCHAR(250)
);


DROP TABLE PRODUCTS;
DROP IF EXISTS TABLE PRODUCTS ;


ALTER TABLE ORDERS ADD COLUMN TOTAL_PRICE NUMERIC(15,2) ;

ALTER TABLE ORDERS DROP COLUMN TOTAL_PRICE ;

ALTER TABLE PRODUCTS ALTER COLUMN CATEGORY TYPE VARCHAR(500) ; 


TRUNCATE TABLE ORDERS; 


DML: 

/*ORDER_ID INTEGER  
	, CUSTOMER_ID  INTEGER 
	, PRODUCT_ID INTEGER 
	, PRICE   NUMERIC(10,2)
	, QUANTITY  INTEGER 
	*/
	
INSERT INTO ORDERS 
VALUES(1, 101, 1001, 10.5, 2 );

INSERT INTO ORDERS(ORDER_ID, PRODUCT_ID, PRICE, QUANTITY) 
VALUES(2, 1001, 10.5, 3 );


UPDATE ORDERS 
SET QUANTITY = 5 
WHERE ORDER_ID = 2 ;

UPDATE ORDERS 
SET QUANTITY = 5 ;


DELETE FROM ORDERS  ;

DELETE FROM ORDERS WHERE ORDER_ID = 2;

-- DQL 
SELECT * FROM ORDERS ;

SELECT ORDER_ID FROM ORDERS ;

SELECT ORDER_ID, PRICE, QUANTITY FROM ORDERS ;

---   AGGREGATE FUNCTIONS 

SELECT SUM(QUANTITY) FROM ORDERS  ;
-- REVENUE 
SELECT SUM(QUANTITY) FROM ORDERS ;
SELECT AVG(QUANTITY) FROM ORDERS ;

SELECT MAX(QUANTITY) FROM ORDERS; 
SELECT MIN(QUANTITY) FROM ORDERS; 

SELECT COUNT(*) FROM ORDERS ; -- TOTAL RECORDS IN THE TABLE 

--  NULL WILL BE IGNORED 

---  COUNT(*) (COMPLETE RECORD) | COUNT(1) (VERY FAST) | COUNT(ORDER_ID) (ORDER_ID NULL WILL BE IGNORE) 

SELECT COUNT(1) FROM ORDERS; 

-- GROUP BY IMPLEMENTATION 

SELECT PRODUCT_ID, SUM(QUANTITY) AS TOTAL_QUANTITY
FROM ORDERS
GROUP BY PRODUCT_ID 
;

SELECT PRODUCT_ID, AVG(QUANTITY) AS TOTAL_QUANTITY 
FROM ORDERS
GROUP BY PRODUCT_ID 
;

SELECT * FROM ORDERS;
SELECT PRODUCT_ID, MIN(QUANTITY) AS TOTAL_REVENUE 
FROM ORDERS
GROUP BY PRODUCT_ID 
;

SELECT PRODUCT_ID, MAX(QUANTITY) AS TOTAL_REVENUE 
FROM ORDERS
GROUP BY PRODUCT_ID 
;

SELECT PRODUCT_ID, COUNT(ORDER_ID) AS TOTAL_REVENUE 
FROM ORDERS
GROUP BY PRODUCT_ID 
;
 
SELECT * FROM ORDERS ;;

SELECT * FROM PRODUCTS ;


-- Insert into PRODUCTS
INSERT INTO PRODUCTS (PRODUCT_ID, PRODUCT_NAME, CATEGORY) VALUES (1, 'Laptop', 'Electronics');
INSERT INTO PRODUCTS (PRODUCT_ID, PRODUCT_NAME, CATEGORY) VALUES (2, 'Phone', 'Electronics');
INSERT INTO PRODUCTS (PRODUCT_ID, PRODUCT_NAME, CATEGORY) VALUES (3, 'Desk Chair', 'Furniture');
INSERT INTO PRODUCTS (PRODUCT_ID, PRODUCT_NAME, CATEGORY) VALUES (4, 'Notebook', 'Stationery'); -- No orders for this product
select * from products;

truncate table orders;
-- Insert into ORDERS
INSERT INTO ORDERS (ORDER_ID, CUSTOMER_ID, PRODUCT_ID, PRICE, QUANTITY) VALUES (101, 1, 1, 1000.00, 1);
INSERT INTO ORDERS (ORDER_ID, CUSTOMER_ID, PRODUCT_ID, PRICE, QUANTITY) VALUES (102, 2, 2, 500.00, 2);
INSERT INTO ORDERS (ORDER_ID, CUSTOMER_ID, PRODUCT_ID, PRICE, QUANTITY) VALUES (103, 3, 3, 150.00, 1);
INSERT INTO ORDERS (ORDER_ID, CUSTOMER_ID, PRODUCT_ID, PRICE, QUANTITY) VALUES (104, 4, 5, 200.00, 1); -- Product does not exist
INSERT INTO ORDERS (ORDER_ID, CUSTOMER_ID, PRODUCT_ID, PRICE, QUANTITY) VALUES (105, 3, 3, 150.00, 2);

select * from orders ;

SELECT O.ORDER_ID, O.PRODUCT_ID, P.PRODUCT_NAME, P.CATEGORY, O.PRICE, O.QUANTITY 
FROM ORDERS O
JOIN PRODUCTS P
ON O.PRODUCT_ID = P.PRODUCT_ID ;


SELECT O.ORDER_ID, O.PRODUCT_ID, P.PRODUCT_NAME, P.CATEGORY, O.PRICE, O.QUANTITY 
FROM ORDERS O
LEFT  JOIN PRODUCTS P
ON O.PRODUCT_ID = P.PRODUCT_ID ;


SELECT O.ORDER_ID, O.PRODUCT_ID, P.PRODUCT_NAME, P.CATEGORY, O.PRICE, O.QUANTITY 
FROM ORDERS O
RIGHT  JOIN PRODUCTS P
ON O.PRODUCT_ID = P.PRODUCT_ID ;

SELECT O.ORDER_ID, P.PRODUCT_ID, P.PRODUCT_NAME, P.CATEGORY, O.PRICE, O.QUANTITY 
FROM ORDERS O
RIGHT  JOIN PRODUCTS P
ON O.PRODUCT_ID = P.PRODUCT_ID ;

SELECT O.ORDER_ID, O.PRODUCT_ID, P.PRODUCT_NAME, P.CATEGORY, O.PRICE, O.QUANTITY 
FROM ORDERS O
FULL  JOIN PRODUCTS P
ON O.PRODUCT_ID = P.PRODUCT_ID ;

SELECT *
FROM ORDERS O ,  PRODUCTS P ;

SELECT *
FROM ORDERS O ,  PRODUCTS P 
WHERE O.PRODUCT_ID = P.PRODUCT_ID;


SELECT O.ORDER_ID, O.CUSTOMER_ID, O.PRODUCT_ID, P.PRODUCT_NAME, P.CATEGORY, O.PRICE, O.QUANTITY
FROM ORDERS O ,  PRODUCTS P 
WHERE O.PRODUCT_ID = P.PRODUCT_ID;

CREATE TABLE CUSTOMERS (
    CUSTOMER_ID INTEGER PRIMARY KEY,
    FIRST_NAME VARCHAR(100),
    LAST_NAME VARCHAR(100),
    EMAIL VARCHAR(255),
    CITY VARCHAR(100),
    COUNTRY VARCHAR(100)
);


INSERT INTO CUSTOMERS (CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, CITY, COUNTRY)
VALUES (1, 'John', 'Smith', 'john.smith@email.com', 'New York', 'USA');

INSERT INTO CUSTOMERS (CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, CITY, COUNTRY)
VALUES (2, 'Sarah', 'Johnson', 'sarah.j@email.com', 'Los Angeles', 'USA');

INSERT INTO CUSTOMERS (CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, CITY, COUNTRY)
VALUES (3, 'Michael', 'Brown', 'michael.b@email.com', 'Chicago', 'USA');

INSERT INTO CUSTOMERS (CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, CITY, COUNTRY)
VALUES (4, 'Emily', 'Davis', 'emily.d@email.com', 'Houston', 'USA');

INSERT INTO CUSTOMERS (CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, CITY, COUNTRY)
VALUES (5, 'David', 'Wilson', 'david.w@email.com', 'Miami', 'USA'); -- No orders

SELECT * FROM CUSTOMERS;

SELECT FIRST_NAME, LAST_NAME,  FIRST_NAME || ' ' || LAST_NAME AS FULL_NAME 
FROM CUSTOMERS ;

SELECT O.ORDER_ID, P.PRODUCT_NAME, P.CATEGORY, C.FIRST_NAME || ' ' || C.LAST_NAME AS FULL_NAME, O.PRICE, O.QUANTITY 
FROM ORDERS O 
JOIN PRODUCTS P  ON O.PRODUCT_ID  = P.PRODUCT_ID  
JOIN CUSTOMERS  C ON C.CUSTOMER_ID = O.CUSTOMER_ID  ;


SELECT O.ORDER_ID, P.PRODUCT_NAME, P.CATEGORY, C.FIRST_NAME || ' ' || C.LAST_NAME AS FULL_NAME, O.PRICE, O.QUANTITY 
FROM ORDERS O 
LEFT JOIN PRODUCTS P  ON O.PRODUCT_ID  = P.PRODUCT_ID  
LEFT JOIN CUSTOMERS  C ON C.CUSTOMER_ID = O.CUSTOMER_ID  ;

DELETE FROM CUSTOMERS WHERE CUSTOMER_ID = 3;

SELECT * FROM CUSTOMERS;

SELECT O.ORDER_ID, P.PRODUCT_NAME, P.CATEGORY, C.FIRST_NAME || ' ' || C.LAST_NAME AS FULL_NAME, O.PRICE, O.QUANTITY 
FROM ORDERS O 
LEFT JOIN PRODUCTS P  ON O.PRODUCT_ID  = P.PRODUCT_ID  
LEFT JOIN CUSTOMERS  C ON C.CUSTOMER_ID = O.CUSTOMER_ID  ;


SELECT O.ORDER_ID, 
P.PRODUCT_NAME, 
P.CATEGORY, 
C.FIRST_NAME || ' ' || C.LAST_NAME AS FULL_NAME, 
O.PRICE, 
O.QUANTITY 
FROM ORDERS O 
LEFT JOIN PRODUCTS P  ON O.PRODUCT_ID  = P.PRODUCT_ID  
JOIN CUSTOMERS  C ON C.CUSTOMER_ID = O.CUSTOMER_ID  ;

drop table a;
CREATE TABLE A (
    ID varchar
);

drop table b;
CREATE TABLE B (
    ID varchar
);

-- Insert into A (duplicates)
INSERT INTO A VALUES (1);
INSERT INTO A VALUES (1);
INSERT INTO A VALUES (1); 
INSERT INTO A VALUES (2);
INSERT INTO A VALUES (3);
INSERT INTO A VALUES ('A');
INSERT INTO A VALUES (null);

select * from a;


-- Insert into A (duplicates)
INSERT INTO B VALUES (1);
INSERT INTO B VALUES (1);
INSERT INTO B VALUES (2);
INSERT INTO B VALUES ('B');
INSERT INTO B VALUES ('A');
INSERT INTO B VALUES (null);
INSERT INTO B VALUES (null);

SELECT * FROM B;

SELECT *
FROM A
JOIN B ON A.ID = B.ID;

SELECT *
FROM A
LEFT JOIN B ON A.ID = B.ID;

SELECT *
FROM A
RIGHT JOIN B ON A.ID = B.ID;

SELECT *
FROM A
FULL JOIN B ON A.ID = B.ID;


---- 42  SQL 
SELECT * FROM ORDERS;
SELECT * FROM PRODUCTS;
SELECT * FROM CUSTOMERS;


-- SUB QUERY 

SELECT CUSTOMER_ID FROM ORDERS;

SELECT * FROM CUSTOMERS 
WHERE CUSTOMER_ID IN (SELECT CUSTOMER_ID FROM ORDERS) ;


SELECT * FROM CUSTOMERS 
WHERE CUSTOMER_ID NOT IN (SELECT CUSTOMER_ID FROM ORDERS) ;  --  UNCORELATED/INDEPENDANT SUB QUERIES 

SELECT CUSTOMER_ID FROM ORDERS;

SELECT * FROM CUSTOMERS  C 
WHERE EXISTS (SELECT 1 FROM ORDERS O  WHERE C.CUSTOMER_ID = O.CUSTOMER_ID) ;   -- CORELATED/DEPENDANT SUB QUERIES 


SELECT * FROM CUSTOMERS  C 
WHERE NOT EXISTS (SELECT 1 FROM ORDERS O  WHERE C.CUSTOMER_ID = O.CUSTOMER_ID) ;


--  CTE 
--  COMMON TABLE EXPRESSION => TEMPORARY RESULT SET INSIDE THE QUERY 
SELECT CUSTOMER_ID, SUM(PRICE) AS TOTAL_SPENT  
FROM ORDERS 
GROUP BY CUSTOMER_ID ;


SELECT CUSTOMER_ID, SUM(PRICE) AS TOTAL_SPENT  
FROM ORDERS 
GROUP BY CUSTOMER_ID 
HAVING SUM(PRICE) >= 500
; --  HAVING CLAUSE WILL COME ALONG WITH GROUP BY CLAUSE  


WITH CTE AS (
SELECT CUSTOMER_ID, SUM(PRICE) AS TOTAL_SPENT  
FROM ORDERS 
GROUP BY CUSTOMER_ID 
)
SELECT * FROM CTE;

WITH CTE AS (
SELECT CUSTOMER_ID, SUM(PRICE) AS TOTAL_SPENT  
FROM ORDERS 
GROUP BY CUSTOMER_ID 
)
SELECT * FROM CTE
WHERE TOTAL_SPENT >= 500
;


WITH SPEND_CTE AS (
SELECT CUSTOMER_ID, SUM(PRICE) AS TOTAL_SPENT  
FROM ORDERS 
GROUP BY CUSTOMER_ID 
)
SELECT C.FIRST_NAME || ' ' || C.LAST_NAME AS FULL_NAME ,  TOTAL_SPENT
FROM SPEND_CTE CT
JOIN CUSTOMERS C ON C.CUSTOMER_ID = CT.CUSTOMER_ID  
WHERE TOTAL_SPENT >= 500
;

-- CASE 
SELECT CUSTOMER_ID, SUM(PRICE) AS TOTAL_SPENT,
CASE WHEN SUM(PRICE) > 500 THEN 'PREMIUM'
	WHEN SUM(PRICE) <= 500 AND SUM(PRICE) >= 300 THEN 'GOLD'
	ELSE  'STANDARD' 
END AS CUSTOMER_CATEGORY
FROM ORDERS 
GROUP BY CUSTOMER_ID ;

WITH CTE AS (
SELECT CUSTOMER_ID, SUM(PRICE) AS TOTAL_SPENT
FROM ORDERS 
GROUP BY CUSTOMER_ID 
)
SELECT CUSTOMER_ID, TOTAL_SPENT, 
CASE WHEN TOTAL_SPENT > 500 THEN 'PREMIUM'
	WHEN TOTAL_SPENT <= 500 AND TOTAL_SPENT >= 300 THEN 'GOLD'
	ELSE  'STANDARD' 
END AS CUSTOMER_CATEGORY
FROM CTE ;


WITH CTE AS (
SELECT CUSTOMER_ID, SUM(PRICE) AS TOTAL_SPENT
FROM ORDERS 
GROUP BY CUSTOMER_ID 
)
SELECT C.CUSTOMER_ID, TOTAL_SPENT, C.FIRST_NAME || ' ' || C.LAST_NAME AS FULL_NAME , 
CASE WHEN TOTAL_SPENT > 500 THEN 'PREMIUM'
	WHEN TOTAL_SPENT <= 500 AND TOTAL_SPENT >= 300 THEN 'GOLD'
	ELSE  'STANDARD' 
END AS CUSTOMER_CATEGORY
FROM CTE
JOIN CUSTOMERS C ON C.CUSTOMER_ID = CTE.CUSTOMER_ID 
ORDER BY TOTAL_SPENT DESC 
;

CREATE TABLE DIM_CUSTOMER (
    CUSTOMER_ID INT PRIMARY KEY,  -- NO NULLS AND NO DUPLICATES
    CUSTOMER_NAME VARCHAR(50),
    EMAIL VARCHAR(100),
    PHONE_NUMBER VARCHAR(20),
    CITY VARCHAR(50),
    STATE VARCHAR(50),
    COUNTRY VARCHAR(50),
    SIGNUP_DATE DATE
);

CREATE TABLE DIM_PRODUCT (
    PRODUCT_ID INT PRIMARY KEY,
    PRODUCT_NAME VARCHAR(100),
    CATEGORY VARCHAR(50),
    PRICE DECIMAL(10, 2),
    STOCK_QUANTITY INT
);

CREATE TABLE DIM_DATE (
    DATE_ID INT PRIMARY KEY,
    DATE DATE,
    YEAR INT,
    MONTH INT,
    MONTH_NAME VARCHAR(20),
    DAY INT,
    QUARTER INT
);

CREATE TABLE FACT_SALES (
    ORDER_ID INT PRIMARY KEY,
    ORDER_DATE DATE,
    CUSTOMER_ID INT,
    PRODUCT_ID INT,
    DATE_ID INT,
    QUANTITY_SOLD INT,
    TOTAL_SALES DECIMAL(10, 2),
    FOREIGN KEY (CUSTOMER_ID) REFERENCES DIM_CUSTOMER(CUSTOMER_ID),  -- SHOULD BE A PRIMARY KEY IN ANOTHER TABLE
    FOREIGN KEY (PRODUCT_ID) REFERENCES DIM_PRODUCT(PRODUCT_ID),
    FOREIGN KEY (DATE_ID) REFERENCES DIM_DATE(DATE_ID)
);

-- Sample Data Insertion
INSERT INTO DIM_CUSTOMER (CUSTOMER_ID, CUSTOMER_NAME, EMAIL, PHONE_NUMBER, CITY, STATE, COUNTRY, SIGNUP_DATE) VALUES
(1, 'John Doe', 'john.doe@example.com', '123-456-7890', 'New York', 'NY', 'USA', '2023-01-15'),
(2, 'Jane Smith', 'jane.smith@example.com', '987-654-3210', 'Los Angeles', 'CA', 'USA', '2023-02-20'),
(3, 'Alice Johnson', 'alice.johnson@example.com', '555-123-4567', 'Chicago', 'IL', 'USA', '2023-03-10'),
(4, 'Bob Brown', 'bob.brown@example.com', '444-555-6666', 'Houston', 'TX', 'USA', '2023-03-05'),
(5, 'Charlie Davis', 'charlie.davis@example.com', '333-222-1111', 'Phoenix', 'AZ', 'USA', '2023-02-12');

INSERT INTO DIM_PRODUCT (PRODUCT_ID, PRODUCT_NAME, CATEGORY, PRICE, STOCK_QUANTITY) VALUES
(1, 'Laptop', 'Electronics', 999.99, 50),
(2, 'Smartphone', 'Electronics', 499.99, 100),
(3, 'Headphones', 'Accessories', 199.99, 200),
(4, 'Office Chair', 'Furniture', 149.99, 30),
(5, 'Coffee Maker', 'Appliances', 89.99, 80);   

INSERT INTO DIM_DATE (DATE_ID, DATE, YEAR, MONTH, MONTH_NAME, DAY, QUARTER) VALUES
(1, '2023-01-01', 2023, 1, 'January', 1, 1),
(2, '2023-01-02', 2023, 1, 'January', 2, 1),
(3, '2023-01-03', 2023, 1, 'January', 3, 1),
(4, '2023-01-04', 2023, 1, 'January', 4, 1),
(5, '2023-01-05', 2023, 1, 'January', 5, 1),
(6, '2023-01-06', 2023, 1, 'January', 6, 1),
(7, '2023-01-07', 2023, 1, 'January', 7, 1),
(8, '2023-01-08', 2023, 1, 'January', 8, 1),
(9, '2023-01-09', 2023, 1, 'January', 9, 1),
(10, '2023-01-10', 2023, 1, 'January', 10, 1),
(11, '2023-01-11', 2023, 1, 'January', 11, 1),
(12, '2023-01-12', 2023, 1, 'January', 12, 1),
(13, '2023-01-13', 2023, 1, 'January', 13, 1),
(14, '2023-01-14', 2023, 1, 'January', 14, 1),
(15, '2023-01-15', 2023, 1, 'January', 15, 1),
(16, '2023-01-16', 2023, 1, 'January', 16, 1),
(17, '2023-01-17', 2023, 1, 'January', 17, 1),
(18, '2023-01-18', 2023, 1, 'January', 18, 1),
(19, '2023-01-19', 2023, 1, 'January', 19, 1),
(20, '2023-01-20', 2023, 1, 'January', 20, 1),
(21, '2023-01-21', 2023, 1, 'January', 21, 1),
(22, '2023-01-22', 2023, 1, 'January', 22, 1),
(23, '2023-01-23', 2023, 1, 'January', 23, 1),
(24, '2023-01-24', 2023, 1, 'January', 24, 1),
(25, '2023-01-25', 2023, 1, 'January', 25, 1),
(26, '2023-01-26', 2023, 1, 'January', 26, 1),
(27, '2023-01-27', 2023, 1, 'January', 27, 1),
(28, '2023-01-28', 2023, 1, 'January', 28, 1),
(29, '2023-01-29', 2023, 1, 'January', 29, 1),
(30, '2023-01-30', 2023, 1, 'January', 30, 1),
(31, '2023-01-31', 2023, 1, 'January', 31, 1),
(32, '2023-02-01', 2023, 2, 'February', 1, 1),
(33, '2023-02-02', 2023, 2, 'February', 2, 1),
(34, '2023-02-03', 2023, 2, 'February', 3, 1),
(35, '2023-02-04', 2023, 2, 'February', 4, 1),
(36, '2023-02-05', 2023, 2, 'February', 5, 1),
(37, '2023-02-06', 2023, 2, 'February', 6, 1),
(38, '2023-02-07', 2023, 2, 'February', 7, 1),
(39, '2023-02-08', 2023, 2, 'February', 8, 1),
(40, '2023-02-09', 2023, 2, 'February', 9, 1),
(41, '2023-02-10', 2023, 2, 'February', 10, 1),
(42, '2023-02-11', 2023, 2, 'February', 11, 1),
(43, '2023-02-12', 2023, 2, 'February', 12, 1),
(44, '2023-02-13', 2023, 2, 'February', 13, 1),
(45, '2023-02-14', 2023, 2, 'February', 14, 1),
(46, '2023-02-15', 2023, 2, 'February', 15, 1),
(47, '2023-02-16', 2023, 2, 'February', 16, 1),
(48, '2023-02-17', 2023, 2, 'February', 17, 1),
(49, '2023-02-18', 2023, 2, 'February', 18, 1),
(50, '2023-02-19', 2023, 2, 'February', 19, 1),
(51, '2023-02-20', 2023, 2, 'February', 20, 1),
(52, '2023-02-21', 2023, 2, 'February', 21, 1),
(53, '2023-02-22', 2023, 2, 'February', 22, 1),
(54, '2023-02-23', 2023, 2, 'February', 23, 1),
(55, '2023-02-24', 2023, 2, 'February', 24, 1),
(56, '2023-02-25', 2023, 2, 'February', 25, 1),
(57, '2023-02-26', 2023, 2, 'February', 26, 1),
(58, '2023-02-27', 2023, 2, 'February', 27, 1),
(59, '2023-02-28', 2023, 2, 'February', 28, 1),
(60, '2023-03-01', 2023, 3, 'March', 1, 1),
(61, '2023-03-02', 2023, 3, 'March', 2, 1),
(62, '2023-03-03', 2023, 3, 'March', 3, 1),
(63, '2023-03-04', 2023, 3, 'March', 4, 1),
(64, '2023-03-05', 2023, 3, 'March', 5, 1),
(65, '2023-03-06', 2023, 3, 'March', 6, 1),
(66, '2023-03-07', 2023, 3, 'March', 7, 1),
(67, '2023-03-08', 2023, 3, 'March', 8, 1),
(68, '2023-03-09', 2023, 3, 'March', 9, 1),
(69, '2023-03-10', 2023, 3, 'March', 10, 1),
(70, '2023-03-11', 2023, 3, 'March', 11, 1),
(71, '2023-03-12', 2023, 3, 'March', 12, 1),
(72, '2023-03-13', 2023, 3, 'March', 13, 1),
(73, '2023-03-14', 2023, 3, 'March', 14, 1),
(74, '2023-03-15', 2023, 3, 'March', 15, 1),
(75, '2023-03-16', 2023, 3, 'March', 16, 1),
(76, '2023-03-17', 2023, 3, 'March', 17, 1),
(77, '2023-03-18', 2023, 3, 'March', 18, 1),
(78, '2023-03-19', 2023, 3, 'March', 19, 1),
(79, '2023-03-20', 2023, 3, 'March', 20, 1),
(80, '2023-03-21', 2023, 3, 'March', 21, 1),
(81, '2023-03-22', 2023, 3, 'March', 22, 1),
(82, '2023-03-23', 2023, 3, 'March', 23, 1),
(83, '2023-03-24', 2023, 3, 'March', 24, 1),
(84, '2023-03-25', 2023, 3, 'March', 25, 1),
(85, '2023-03-26', 2023, 3, 'March', 26, 1),
(86, '2023-03-27', 2023, 3, 'March', 27, 1),
(87, '2023-03-28', 2023, 3, 'March', 28, 1),
(88, '2023-03-29', 2023, 3, 'March', 29, 1),
(89, '2023-03-30', 2023, 3, 'March', 30, 1),
(90, '2023-03-31', 2023, 3, 'March', 31, 1);

INSERT INTO FACT_SALES (ORDER_ID, ORDER_DATE, CUSTOMER_ID, PRODUCT_ID, DATE_ID, QUANTITY_SOLD, TOTAL_SALES) VALUES
(1, '2023-01-15', 1, 1, 15, 1, 999.99),
(2, '2023-01-20', 2, 2, 20, 2, 999.98),
(3, '2023-02-10', 3, 3, 41, 3, 599.97),
(4, '2023-02-15', 4, 4, 46, 1, 149.99),
(5, '2023-03-05', 5, 5, 65, 4, 359.96),
(6, '2023-03-10', 1, 1, 70, 2, 1999.98),
(7, '2023-03-15', 2, 2, 75, 1, 499.99),
(8, '2023-03-20', 3, 3, 80, 5, 999.95),
(9, '2023-03-25', 4, 4, 85, 3, 449.97),
(10, '2023-03-30', 5, 5, 90, 2, 179.98),
(11, '2023-01-18', 1, 2, 18, 1, 499.99),
(12, '2023-01-22', 2, 3, 22, 2, 399.98),
(13, '2023-02-12', 3, 4, 44, 1, 149.99),
(14, '2023-02-18', 4, 5, 48, 3, 269.97),
(15, '2023-03-08', 5, 1, 68, 1, 999.99),
(16, '2023-03-12', 1, 2, 72, 4, 1999.96),
(17, '2023-03-18', 2, 3, 78, 2, 399.98),
(18, '2023-03-22', 3, 4, 82, 3, 449.97),
(19, '2023-03-28', 4, 5, 88, 1, 89.99),
(20, '2023-03-31', 5, 1, 90, 2, 1999.98);



SELECT * FROM DIM_CUSTOMER;
SELECT * FROM DIM_PRODUCT;
SELECT * FROM DIM_DATE;
SELECT * FROM FACT_SALES;

--  DAILY REVENUE 
SELECT ORDER_DATE, SUM(TOTAL_SALES) AS TOTAL_SALES 
FROM FACT_SALES
GROUP BY ORDER_DATE 
ORDER BY ORDER_DATE ;

SELECT CURRENT_DATE;
SELECT DATE_TRUNC('month',CURRENT_DATE);

SELECT DATE_TRUNC('year',CURRENT_DATE);

SELECT DATE_TRUNC('MONTH',ORDER_DATE) AS MONTHLY_REVENUE, SUM(TOTAL_SALES) AS TOTAL_SALES 
FROM FACT_SALES 
GROUP BY DATE_TRUNC('MONTH',ORDER_DATE) 
ORDER BY DATE_TRUNC('MONTH',ORDER_DATE) ;

-- MONTHLY REVENUE 
SELECT DD.month_name AS MONTHLY_REVENUE, SUM(TOTAL_SALES) AS TOTAL_SALES 
FROM FACT_SALES SF  
JOIN DIM_DATE DD ON DD.date_id = SF.date_id
GROUP BY DD.month_name
ORDER BY TOTAL_SALES DESC ;

-- TOP SELLING PRODUCTS 
SELECT 
SF.PRODUCT_ID, SUM(SF.quantity_sold) AS QUANTITY_SOLD, SUM(SF.total_sales) AS TOTAL_SALES
FROM FACT_SALES SF
GROUP BY SF.PRODUCT_ID 
ORDER BY TOTAL_SALES DESC, QUANTITY_SOLD DESC
;

SELECT 
SF.PRODUCT_ID, PD.product_name, SUM(SF.quantity_sold) AS QUANTITY_SOLD, SUM(SF.total_sales) AS TOTAL_SALES
FROM FACT_SALES SF
JOIN DIM_PRODUCT PD ON SF.PRODUCT_ID = PD.PRODUCT_ID  
GROUP BY SF.PRODUCT_ID , PD.product_name
ORDER BY TOTAL_SALES DESC, QUANTITY_SOLD DESC
;

-- CUSTOMER LIFE TIME VALUE   (clv)

SELECT SF.CUSTOMER_ID, DC.CUSTOMER_NAME, SUM(SF.total_sales) AS TOTAL_SALES
FROM FACT_SALES SF
JOIN DIM_CUSTOMER DC ON DC.CUSTOMER_ID = SF.CUSTOMER_ID
GROUP BY SF.CUSTOMER_ID, DC.CUSTOMER_NAME
ORDER BY 3 DESC;


---  CUSTOMERS WITH REPEATED ORDERS 
SELECT 
CUSTOMER_ID, COUNT(ORDER_ID)
FROM FACT_SALES SF
GROUP BY CUSTOMER_ID 
HAVING COUNT(ORDER_ID) > 1 ;


-- CUSTOMER_METRICS 
SELECT 
SF.CUSTOMER_ID, CD.CUSTOMER_NAME, CD.CITY, CD.STATE, CD.COUNTRY, CD.signup_date,
MAX(ORDER_DATE) AS LAST_PURCHASED_DT, COUNT(ORDER_ID) AS COUNT_OF_ORDERS, SUM(TOTAL_SALES) AS TOTAL_REVENU
FROM FACT_SALES SF 
JOIN DIM_CUSTOMER CD  ON SF.CUSTOMER_ID = CD.CUSTOMER_ID 
GROUP BY SF.CUSTOMER_ID, CD.CUSTOMER_NAME, CD.CITY, CD.STATE, CD.COUNTRY, CD.signup_date;